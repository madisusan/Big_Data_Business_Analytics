...
df <- data.table(readxl::read_xlsx('/Volumes/DOCUMENTS/IE/Big Data/TERM 2/data viz/AMEX 2016 Spending.xlsx'), 
                 row.names = seq(1,110))
...
Starting this section, we take a look at Andy's expenses. The first chart we create plots time vs money spent. This is done in order to assess the full shape of the data, and to see if the large transaction represented an outlier or fit with the suspect's usual purchasing activity. This chart reveals that the purchase was indeed an outlier.
...
(bars <- ggplot(df) + geom_histogram(aes(x=Category),stat='count', fill = main2_color) + xlab("") +
  ylab("Number of purchases") +  labs(title = "Number of purchases per category"))
...
## Auto expenses - Tufte Chart:

We take a closer look at Andy's expenses; targeting his auto expenses. A tufte chart is constructed using Andy's spendings on auto transportation in the US, UK, and Europe.
...
ggplot(df_tufte, aes(Country_group,Amount)) + 
  ...
  labs(title = 'Normal spendings per region without the purchase of the car') + xlab("")
...
Our focus shifts towards Andy's expenses over time for different categories. We look at how much money he has spent on specific categories during the year. A quarterly analysis is then undertaken in order to determine if there had been a pattern to the suspect's purchases leading up to the anomalous transaction. This reveals a sudden unexplained shift in behaviour, as the suspect appears to have developed an overnight interest in purchasing expensive tickets for music and entertainment events.

The data is presented on a waffle chart.
...
ggplot(df, aes(x=Subcategory)) + geom_histogram(stat='count')
